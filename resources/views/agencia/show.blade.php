...
<h1>Agencia  @if($agencia) {{$agencia->Agencia}} @endif <a href="{{ route('agencia.index') }}" class="btn btn-sm btn-primary">Agencias</a></h1>

@stop
@section('content')
<div class="table-responsive">
    <table class="table table-striped table-hover table-bordered" id="tabla_agencia">
        <thead>
            <tr>
...
<script>
    $('#tabla_agencia').DataTable({
        serverSide: false,
        ajax: `{{ config('app.url') }}/admin/agencia/{{$id}}/get-list`,
        bSort: true,
        columns: [
